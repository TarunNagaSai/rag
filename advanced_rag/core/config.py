...
class Settings:
    ...
    gen_model: str = field(
        default_factory=lambda: _env("ARAG_GEN_MODEL", "gemini-3.1-flash-lite")
    )
    # Stronger model reserved for hard multi-hop reasoning / final synthesis.
    reasoning_model: str = field(
        default_factory=lambda: _env("ARAG_REASONING_MODEL", "gemini-3.1-flash-lite")
    )
    embed_model: str = field(
        default_factory=lambda: _env("ARAG_EMBED_MODEL", "gemini-embedding-002")
    )
    ...
    embed_batch_size: int = field(
        default_factory=lambda: int(_env("ARAG_EMBED_BATCH", "32"))
    )
